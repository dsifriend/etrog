...
import type { LanguageTag, URI, XSDDate } from "../src/types/index.js";
import { ETROG_NS_UUID, mintIRI, mintURI } from "../src/types/uri.js";
...
describe("ETROG_NS_UUID", () => {
	test("is the Etrog project namespace URN", () => {
		expect(ETROG_NS_UUID as string).toBe(
			"urn:uuid:96f16397-6054-4c94-8ef6-c01603f158cc",
		);
	});
});
...
describe("mintURI", () => {
	...
	test("throws when namespace does not use urn:uuid prefix", async () => {
		await expect(
			mintURI("https://example.org/not-a-uuid-namespace" as URI, "test"),
		).rejects.toThrow(TypeError);
	});

	test("throws when namespace UUID is malformed", async () => {
		await expect(
			mintURI("urn:uuid:not-a-valid-uuid" as URI, "test"),
		).rejects.toThrow(TypeError);
	});
	...
	test("sets variant bits correctly", async () => {
		const uri = await mintURI(ETROG_NS_UUID, "hello");
		const parts = uri.replace("urn:uuid:", "").split("-");
		const variantByte = Number.parseInt(parts[3].slice(0, 2), 16);
...
describe("mintIRI", () => {
	...
	test("inserts a slash when base has no trailing slash or hash", () => {
		const iri = mintIRI("https://example.org/lex", "house");
		expect(iri as string).toBe("https://example.org/lex/house");
	});

	test("does not insert a slash when base ends with hash", () => {
		const iri = mintIRI("https://example.org/lex#", "house");
		expect(iri as string).toBe("https://example.org/lex#house");
	});
});
