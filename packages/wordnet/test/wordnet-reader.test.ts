...
describe("readOewnTurtle", () => {
	...
	test("accepts both https and http wn namespace variants", () => {
		expect(
			isWnUri("https://globalwordnet.github.io/schemas/wn#hypernym" as URI),
		).toBe(true);
		expect(
			isWnUri("http://globalwordnet.github.io/schemas/wn#hypernym" as URI),
		).toBe(true);

		expect(
			isWnCategoryRelation({
				"vartrans:category":
					"http://globalwordnet.github.io/schemas/wn#hypernym" as URI,
			}),
		).toBe(true);
	});
});
